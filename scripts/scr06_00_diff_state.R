...
# fig23 <- plotMedExprs(sce, k = "merging1", facet = "cluster_id", shape_by = "patient_id") # Deprecated function. 
fig23 <- plotPbExprs(sce, k = "merging1", facet = "cluster_id")                                
...
fig24_1 <- plotDiffHeatmap(sce, ds_res1, top_n = 10, order = TRUE, 
...
fig24_2 <- plotDiffHeatmap(sce, ds_res2, top_n = 10, order = TRUE, 
                         th = FDR_cutoff, normalize = TRUE, hm1 = FALSE)
dev.off()
rm(fig24_2)
gc()
...
daf <- mergeClusters(sce, k = "meta20", id = "merging_all",
...
fig25 <- plotMedExprs(sce, features = "state", shape_by = "patient_id")
...
# Fit linear model.
ds_res3 <- diffcyt(sce, formula = ds_formula1, contrast = contrast,               
                   analysis_type = "DS", method_DS = "diffcyt-DS-LMM",            
                   clustering_to_use = "merging_all", verbose = TRUE)

# fit linear mixed model with patient ID as random effect          
ds_res4 <- diffcyt(sce, formula = ds_formula2, contrast = contrast,                    
                   analysis_type = "DS", method_DS = "diffcyt-DS-LMM",            
                   clustering_to_use = "merging_all", verbose = TRUE)
...
fig26_1 <- plotDiffHeatmap(sce, rowData(ds_res3$res), all = TRUE, hm1 = FALSE)
dev.off()
rm(fig26_1)
gc()

pdf(file = file.path(figures_folder, "figure26_lmm_4.pdf"), width = 28, height = 14)
fig26_2 <- plotDiffHeatmap(sce, rowData(ds_res4$res), all = TRUE, hm1 = FALSE)
dev.off()
rm(fig26_2)
gc()
